...
export const stringWithInvalidChar = '1.5..2.84..63.12.7.|..5.....9..1....8.2.3674.3.7.2..9D47...8..1..16....926914.37.';
const stringMoreThan81 = '1.5..2.84..63.12.7.9..5.....9..9....8.2.3674.3.7.2..9947...8..9..16....9269314.37.';
export const defaultPuzzle = "..9..5.1.85.4....2432......1...69.83.9.....6.62.71...9......1945....4.37.4.3..6..";
...
suite('Unit Tests', () => {
  ...
  test('#10 Valid puzzle strings pass the solver', (done) => {
    assert.isNotFalse(solver.solve(puzzlesAndSolutions[3][0]));
    done();
  });

  test('#11 Invalid puzzle strings fail the solver', (done) => {
    assert.isFalse(solver.solve('1231231.41241.4124'));
    done();
  });
